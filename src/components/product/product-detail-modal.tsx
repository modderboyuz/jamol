...
export function ProductDetailModal({ 
  ...
  const { data: similarProducts, isLoading: loadingSimilar } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allProducts.filter((p: any) => p.id !== product.id).slice(0, 4);
    },
  });
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[95vh] overflow-y-auto p-0">
        <div className="relative">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-10 p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-sm"
          >
